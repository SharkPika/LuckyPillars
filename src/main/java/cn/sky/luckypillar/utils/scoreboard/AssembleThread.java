...
public class AssembleThread extends Thread {
    ...
    AssembleThread(Assemble assemble) {
        ...
        if (protocolLib != null && protocolLib.getDescription().getVersion().startsWith("5")) {
            taskId = Bukkit.getScheduler().runTaskTimer(BedWars.getInstance(), this, assemble.getTicks(), assemble.getTicks()).getTaskId();
        } else {
            taskId = Bukkit.getScheduler().runTaskTimerAsynchronously(BedWars.getInstance(), this, assemble.getTicks(), assemble.getTicks()).getTaskId();
        }*/
    }
